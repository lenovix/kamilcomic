  ...
  <main className="p-4 sm:p-6 mx-auto space-y-16 bg-gray-50 min-h-screen">
          ...
          >
            ...
            <div className="p-3 space-y-1">
              <h3 className="text-sm font-semibold text-gray-800 group-hover:text-blue-600 truncate">
                {typeof comic.title === 'string' ? comic.title : comic.title?.[0]}
              </h3>
              <span className="text-xs text-gray-400">{comic.uploaded}</span>
              {isClient && (
                <div className="flex items-center text-yellow-500 text-xs">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <span key={i}>{getAverageRating(comic.slug) > i ? '★' : '☆'}</span>
                  ))}
                  <span className="ml-1 text-gray-500">({getAverageRating(comic.slug)}/5)</span>
                </div>
              )}
            </div>
          </Link>
    ...
    <section>
      ...
      {filteredComics.length === 0 ? (
        <p className="text-gray-500">Tidak ditemukan komik dengan judul tersebut.</p>
      ) : (
            ...
            >
              ...
              <div className="p-3 flex-1 flex flex-col justify-between space-y-1">
                ...
                <p className="text-xs text-gray-500 truncate">
                  {Array.isArray(comic.tags) ? comic.tags.join(", ") : comic.tags}
                </p>
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full w-max ${
                  (comic.status === "Completed" || comic.status?.[0] === "Completed")
